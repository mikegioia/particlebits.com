...
Inside `article.phtml`, helper closures are in scope: `$e(string)` (HTML-escape and echo), `$a(key)` (echo asset URL from the `assets` map), `$d(key)` (echo value from `data`), `$wl(key)` (echo URL from `weblinks`), `$al(year, slug)` (echo an env-correct URL to another article by year and slug — the year is the target article's URL year, i.e. the year of its `date` field in `about.json`, which can differ from its source directory year). Titles and snippets support a mini inline markup: `_..._` → `<em>`, `*...*` → `<strong>`, `{}` → `/`.
