...
Inside `article.phtml`, helper closures are in scope: `$e(string)` (HTML-escape and echo), `$a(key)` (echo asset URL from the `assets` map), `$d(key)` (echo value from `data`), `$wl(key)` (echo URL from `weblinks`). Titles and snippets support a mini inline markup: `_..._` → `<em>`, `*...*` → `<strong>`, `{}` → `/`.
